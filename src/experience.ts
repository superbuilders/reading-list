...
scene.add(key);
const fill = new THREE.DirectionalLight(0xa9b6ff, 0.2);
...
const BOOKS = [
  {
    ...
    /* buybooksindia's CDN sends no Access-Control-Allow-Origin, so this
       cover is served from public/ instead of hotlinked */
...
function populatePanel(cfg) {
  ...
  audioLink.setAttribute(
    "aria-label",
    `Find the audiobook of ${cfg.title} by ${cfg.author}`,
  );
  let s = "";
  for (let i = 0; i < 5; i++)
